...
class PPMatting(nn.Layer):
    ...
    def forward(self, inputs, mode = 'train'):
        x = inputs['img']
        guided_map = inputs['gm'][:,2,:,:].unsqueeze(1)
        ...
        if self.training:
            ...
            semantic_map = 1 - (paddle.argmax(semantic_map, axis=1, keepdim=True) / (semantic_map.shape[1] - 1))
            # semantic_map = paddle.argmax(semantic_map, axis=1)
