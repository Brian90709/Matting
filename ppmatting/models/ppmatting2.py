...
from ppmatting.models import ConvGuidedFilter


def get_guided_map(index,gm,mode='edge'):
    b,c,h,w = gm.shape
    if mode == 'edge':
        transition_region = (index==1)
        edge_map = gm * transition_region
        grad_x = edge_map[:,:,:,:-1] - edge_map[:,:,:,1:]
        grad_y = edge_map[:,:,:-1,:] - edge_map[:,:,1:,:]
        grad_x = paddle.mean(grad_x ** 2,axis=(2,3))
        grad_y = paddle.mean(grad_y ** 2,axis=(2,3))
        cue = grad_x + grad_y

    elif mode == 'rgb':
        sky_region = (index==2)
        sky_ill = gm * sky_region
        cue = paddle.mean(sky_ill,axis=(2,3))

    arg = paddle.argmax(cue,axis=1)
    guided_map = paddle.zeros([b,1,h,w])
    for i in range(b):
        guided_map[i,:,:,:] += gm[i,arg[i],:,:]
...
class PPMatting(nn.Layer):
    ...
    def __init__(self, backbone, pretrained=None, gf = True):
        ...
        self.hrdb = HRDB(
            ...
            gf_index=[0, 2, 4])
        self.conv_gf = ConvGuidedFilter()
        self.init_weight()

    def forward(self, inputs, mode = 'train', gf_mode = 'rgb'):
        x = inputs['img']
        gm = inputs['gm']
        ...
        guided_map = get_guided_map(index,gm,mode=gf_mode)
        ...
        # if self.gf == True:
        #     detail_map = self.conv_gf(guided_map, detail_map)
        fusion = self.fusion(semantic_map, detail_map)
        if self.gf == True:
            fusion = self.conv_gf(guided_map, fusion)
        if self.training:
            logit_dict = {
            ...
            #sky:2 unknown:1 nosky:0
            semantic_map = 1 - (index / (semantic_map.shape[1] - 1))
            return [fusion, detail_map, semantic_map]
        else:
            return fusion
