...
class Compose:
    ...
    def __call__(self, data):
        ...
        data['gm'] = np.transpose(data['gm'], (2, 0, 1))
...
@manager.TRANSFORMS.add_component
class LoadImages:
    def __init__(self, to_rgb=True, gf=True):
        self.to_rgb = to_rgb
        self.gf = gf
    ...
    def __call__(self, data):
        ...
        if self.gf:
            # data['gm'] = data['img'][:,:,2][:,:,np.newaxis].astype(np.float32)/255.0
            data['gm'] = data['img'].astype(np.float32)/255.0
...
@manager.TRANSFORMS.add_component
class Resize:
    def __init__(self, target_size=(512, 512), random_interp=False, gf = True):
    ...
    def __call__(self, data):
        ...
        if self.gf == True:
            data['gm'] = functional.resize(data['gm'], self.target_size, interp)
        for key in data.get('gt_fields', []):
            if key == 'trimap':
...
class ResizeToIntMult:
    ...
    def __init__(self, mult_int=32, gf = True):
    ...
    def __call__(self, data):
        ...
        if self.gf == True:
            data['gm'] = functional.resize(data['gm'], (rw, rh))
            # data['gm'] = data['img']
        for key in data.get('gt_fields', []):
            if key == 'trimap':
...
class RandomCrop:
    ...
    def __init__(self, crop_size=((320, 320), (480, 480), (640, 640)), gf=True):
    ...
    def __call__(self, data):
        ...
        data['img'] = data['img'][start_h:end_h, start_w:end_w]
        if self.gf == True:
            data['gm'] = data['gm'][start_h:end_h, start_w:end_w]
...
class LimitShort:
    ...
    def __init__(self, max_short=None, min_short=None, gf=True):
    ...
    def __call__(self, data):
        ...
        if target != short_edge:
            ...
            if self.gf == True:
                data['gm'] = functional.resize_short(data['gm'], target)
            for key in data.get('gt_fields', []):
                if key == 'trimap':
...
class RandomHorizontalFlip:
    ...
    def __init__(self, prob=0.5, gf=True):
        self.prob = prob
        self.gf = gf
    def __call__(self, data):
        if random.random() < self.prob:
            ...
            if self.gf:
                data['gm'] = functional.horizontal_flip(data['gm'])
        return data
