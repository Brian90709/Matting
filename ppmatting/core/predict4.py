...
from ppmatting.models import GuidedFilter
...
def save_result(alpha,semantic_map,detail_map,path, im_path, trimap=None, fg_estimate=True, fg=None):
    """
    Save alpha and rgba.
...
def preprocess(img, transforms, trimap=None):
    ...
    data['gm'] = paddle.to_tensor(data['gm'])
    data['gm'] = data['gm'].unsqueeze(0)
    ...
    return data
def get_cue(index,gm,mode='edge'):
    if mode == 'edge':
    ...
    with open(output_txt_path, 'w') as output_file:
        with paddle.no_grad():
            for i, im_path in enumerate(img_lists[local_rank]):
                ...
                cue = get_cue(index, img)
                cue = [cue[0,i].item() for i in range(3)]
                img_name = im_path.split('/')[-1]
                print(img_name)
                output_file.write(f"{img_name} r:{cue[0]}, g:{cue[1]}, b:{cue[2]}\n")
